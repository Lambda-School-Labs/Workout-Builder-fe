...
const Wrapper = ({ children, editing, className }) => {
  return editing
...
function Exercise({ id, navigate, location }){
  ...
  return(
    <div className="fixed top-0 left-0 w-full h-screen z-10 lg:flex lg:items-center lg:justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}>
      <Wrapper editing={editing} className="bg-white h-full overflow-y-scroll flex flex-col px-4 py-5 lg:h-auto lg:rounded">
        {editing ? (
          <input
            type="text"
            name="name"
          ...
          <div className="flex justify-between items-center">
            ...
            <svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-2 cursor-pointer hidden lg:block" onClick={handleBack}>
        ...
        <div className="py-2 lg:py-1 flex-grow">
        ...
        <div className="flex justify-between">
          <button type="button" onClick={editing ? cancelEdit : handleBack} className={`py-3 w-5/12 rounded bg-white text-blaze-orange border border-blaze-orange ${editing ? '' : 'lg:hidden'}`}>
            {editing ? 'Cancel' : 'Back'}
          </button>
